...
e = x.to("eV", equivalencies=u.spectral())
dfde = (y / e ** 2).to("1 / (cm2 eV s)")
dfde_err = (y_err / e ** 2).to("1 / (cm2 eV s)")
sed = data.Data1D("sed", e[~UL], dfde[~UL], staterror=dfde_err[~UL])
...
# - blob parameters
agnpy_ssc.log10_R_b = np.log10(R_b.to_value("cm"))
agnpy_ssc.log10_R_b.freeze()
agnpy_ssc.delta_D = 20
agnpy_ssc.delta_D.freeze()
agnpy_ssc.log10_B = np.log10(B.to_value("G"))
agnpy_ssc.log10_B.freeze()
# - EED
agnpy_ssc.log10_k_e = -5.5
agnpy_ssc.log10_gamma_b = np.log10(1e4)
agnpy_ssc.p1 = 1.8
agnpy_ssc.p2 = 2.9
...
# plot inital model
energy = np.logspace(-6, 16, 200)
plt.loglog(energy, agnpy_ssc(energy), color="crimson")
plt.errorbar(
    sed.x, sed.y.value, yerr=sed.get_error().value, ls="", marker=".", color="k"
)
plt.show()
...
min_x = (1e11 * u.Hz).to("eV", equivalencies=u.spectral())
max_x = (1e30 * u.Hz).to("eV", equivalencies=u.spectral())
...
R_b = 10 ** agnpy_ssc.log10_R_b.val * u.cm
